...
在完成了[《网络通信层 》](/netty_source_code_parsing/network_program/socket_network_programming)的学习之后，我们开始学习 Netty 的启动引导层

先来引出 Netty 中使用的 **主从 Reactor IO 线程模型**

<img src="https://echo798.oss-cn-shenzhen.aliyuncs.com/img/202411151944519.png?x-oss-process=image/watermark,image_aW1nL3dhdGVyLnBuZw==,g_nw,x_1,y_1" alt="image-20241115194426286" style="zoom: 33%;" />

它在实现上与 **Doug Lea** 在 [《Scalable IO in Java》](https://gee.cs.oswego.edu/dl/cpjslides/nio.pdf) 中提到的经典 **主从 Reactor 多线程模型** 有所差异
...
Main Reactor Group 中的 Reactor 数量取决于服务端需要监听的端口数。通常情况下，服务端程序只会监听**一个**端口，因此 Main Reactor Group 中仅会有**一个**Main Reactor 线程 来负责最重要的工作：
...
* **线程安全**：每个客户端的 `SocketChannel` 只分配给一个固定的 Sub Reactor，这样在处理该客户端的 I/O 操作时，不会涉及多个线程的竞争，保证了 I/O 处理的线程安全性。
...
<img src="https://echo798.oss-cn-shenzhen.aliyuncs.com/img/1730000821151-f14c54b4-0fa7-40cb-bb3f-b8fc31e4fcb7.png" alt="img" style="zoom:67%;" />
...
`nThreads` 参数表示当前要创建的 Reactor 线程组 内包含多少个 Reactor 线程。如果不指定 `nThreads` 参数，则会采用默认的 Reactor 线程个数，用 `0` 表示。
...
- **`RejectedExecutionHandler`**：当向 Reactor 添加异步任务失败时，执行的拒绝策略。
- **`SelectorProvider selectorProvider`**：由于 Reactor 线程模型对应的 IO 模型是 IO 多路复用模型，所以在 JDK NIO 中对应实现为 `java.nio.channels.Selector`（即在[《IO 多路复用》](/netty_source_code_parsing/network_program/io_multiplexing)提到的 select、poll、epoll）。每个 Reactor 包含一个 Selector，用于轮询注册在该 Reactor 上的所有 Channel 的 IO 事件。`SelectorProvider` 用于创建 Selector。
...
<img src="https://echo798.oss-cn-shenzhen.aliyuncs.com/img/202410311531612.png" alt="image-20241031153130559" style="zoom: 67%;" />
...
`MultithreadEventExecutorGroup`这里就是本文的核心，主要用来定义创建和管理`Reactor`的行为。
...
3. 如果以上两种方式均未被定义，那么就采用`SelectorProvider`系统默认实现`sun.nio.ch.DefaultSelectorProvider`。echo 当前使用的操作系统是`MacOS`，从源码中我们可以看到自动适配了`KQueue`实现。
...
::: info 不同操作系统中，JDK 对于 `DefaultSelectorProvider` 的实现有所不同：
...
<img src="https://echo798.oss-cn-shenzhen.aliyuncs.com/img/202411152023901.png?x-oss-process=image/watermark,image_aW1nL3dhdGVyLnBuZw==,g_sw,x_1,y_1" alt="image-20241115202339806" style="zoom:33%;" />
